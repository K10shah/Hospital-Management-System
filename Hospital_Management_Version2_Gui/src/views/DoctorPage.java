...
public class DoctorPage extends JFrame 
{
	...
	private void initComponents()
	{
		...
		lblDiag.setVisible(false);
		lblDiag.setBounds(24, 74, 126, 20);
		pnlUpdateRec.add(lblDiag);

		txtDiag = new JTextArea();
		txtDiag.setVisible(false);
		txtDiag.setBounds(24, 93, 347, 54);
		pnlUpdateRec.add(txtDiag);

		lblPres = new JLabel("Enter the prescription");
		lblPres.setVisible(false);
		lblPres.setBounds(24, 164, 126, 20);
		pnlUpdateRec.add(lblPres);

		txtPres = new JTextArea();
		txtPres.setVisible(false);
		txtPres.setBounds(24, 182, 347, 54);
		...
		btnUpdate.setBounds(24, 247, 89, 23);
		pnlUpdateRec.add(btnUpdate);
		
		lblamountMe = new JLabel("");
		lblamountMe.setBounds(252, 77, 46, 14);
		pnlUpdateRec.add(lblamountMe);
	...
	private void createEvents()
	{
			...
			public void actionPerformed(ActionEvent e) 
			{
				...
				if(username.length() > 0)
				{
					if(loginInfo.getPatients().containsKey(username))
					{
						...
						txtPatientRec.setText(p.ViewRecords());
					}
					else
					{
		...
		btnUpSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(username.length() > 0)
				{
					...
					{
						lblUpdateStat.setVisible(false);
						lblDiag.setVisible(true);
						lblPres.setVisible(true);
						txtDiag.setVisible(true);
						txtPres.setVisible(true);
		...
		btnAmount.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String username = txtUpSearch.getText();
				double amount = 0.0;
				try
				{
					amount = Double.parseDouble(txtAmount.getText());
					Patient p = loginInfo.getPatients().get(username);
					p.setAccountBalance(amount);
				}
				catch(Exception ex)
				{
					lblamountMe.setText("Enter proper amount please");
				}
